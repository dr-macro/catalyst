...
if __name__ == "__main__":
    ...
    # Ontology diff charts (pre-generated by build_ontology.py)
    from ontology_viz import get_chart_paths
    ontology_charts = get_chart_paths(today_str)
    ontology_block = ""

    ont_diff_path = ontology_charts.get("diff")
    ont_theme_path = ontology_charts.get("theme")

    if ont_diff_path:
        inline_images.append((str(ont_diff_path), "ontology_diff_chart"))
        ontology_block += (
            '<h2>Narrative Ontology Shift</h2>'
            '<p>Knowledge graph of entities and relationships — colour-coded changes '
            'from previous day (green = new, red = dropped, orange = weakening, '
            'bright green = strengthening).</p>'
            '<p><img src="cid:ontology_diff_chart" alt="Ontology diff" style="max-width:100%;" /></p>'
        )
        print(f"Ontology diff chart attached: {ont_diff_path}")

    if ont_theme_path:
        inline_images.append((str(ont_theme_path), "ontology_theme_chart"))
        ontology_block += (
            '<h2>Narrative Theme Drift</h2>'
            '<p>Theme prominence: previous day (grey) vs today (colour-coded).</p>'
            '<p><img src="cid:ontology_theme_chart" alt="Theme drift" style="max-width:100%;" /></p>'
        )
        print(f"Ontology theme chart attached: {ont_theme_path}")

    # Ontology plain-text diff summary
    diff_txt_path = Path(f"ontologies/diff_{today_str}.txt")
    if diff_txt_path.exists():
        diff_summary = diff_txt_path.read_text(encoding="utf-8")
        ontology_block += (
            '<h3>Ontology change summary</h3>'
            f'<pre style="font-size:12px;">{html.escape(diff_summary)}</pre>'
        )
